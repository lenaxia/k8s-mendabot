...
the agent writes `mechanic-dryrun-<fp12>` at the end of a dry-run job; the controller
...
1. `/mechanic-cfg/dry-run` sentinel file (tamper-proof read-only emptyDir mount)
...
**Location:** `charts/mechanic/templates/clusterrole-agent.yaml`
...
| `charts/mechanic/templates/clusterrole-agent.yaml` | Removed `configmaps` and `secrets` from cluster-wide read rule |
...
- CI will build and push `ghcr.io/lenaxia/mechanic-agent:v0.3.20` on tag push.
- After CI completes:
  ```
  helm upgrade mechanic charts/mechanic --reuse-values
  ```
